...
public static class ApiRepository
{
    ...
    public static async Task<(string, string, Token)> AuthenticateClientAndUser(LoginUser loginUser)
    {
        ...
        var accessToken = new Token() { Value = result.AccessToken, ExpirationDate = result.AccessTokenExpirationDate };
        var identityToken = result.IdentityToken;
        var refreshToken = result.RefreshToken;

        return (identityToken, refreshToken, accessToken);
    }

    public static async Task<List<LoginUser>> GetUsersAsync(string token)
    {
        ...
        var loginUsers = JsonSerializer.Deserialize<List<LoginUser>>(resultJsonString.Replace("login", "Login").Replace("password", "Password"));

        return loginUsers ?? new List<LoginUser>();
    }

    public static async Task<LoginUser> CreateUserAsync(LoginUser loginUser)
    {
        using var client = Provider.GetRequiredService<HttpClient>();
    ...
    public static async Task<(string, string, string)> RefreshTokenAsync(string refreshToken)
    {
        var service = Provider.GetRequiredService<OpenIddictClientService>();
        ...
        return (identityToken, accessToken, refreshTokenResult); ;


        //var httpClient = new HttpClient();
        //var request = new HttpRequestMessage(HttpMethod.Post, $"https://localhost:7292/api/client/refresh-token?token={token}");
